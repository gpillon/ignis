# Build the one-off vision fixture recorder (GitHub #176) against the
# reference's existing `build-ninja` tree: the same static libraries, include
# directories and compile flags as its `ninfer_qwen3_6_frontend_test`.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File tools/vision-fixtures/build.ps1
#
# Produces `$OutDir\record.exe` next to the FFmpeg DLLs it loads.
param(
    [string]$Ninfer = 'F:\ai\q38\ninfer',
    [string]$OutDir = (Join-Path $env:TEMP 'ignis-vision-recorder'),
