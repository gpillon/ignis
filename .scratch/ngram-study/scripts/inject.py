...
class Injector:
    ...
    def __init__(self, model, layer, positions, vectors, alpha,
                 cos_threshold=None, force=False):
    ...
    def _hook(self, _module, _args, out):
        ...
        v_unit = v / v.norm(dim=1, keepdim=True).clamp_min(1e-9)
        local_norm = local.norm(dim=1, keepdim=True)
        cos = (local / local_norm.clamp_min(1e-9) * v_unit).sum(dim=1)
...
@torch.no_grad()
def cosine_probe(model, layer, positions, vectors, input_ids):
