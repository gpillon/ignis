...
/**
 * A `noul`: the option that won, in the caller's own words.
 *
 * The answer and not the probability, because "0.862" is not what was
 * decided — it is how sure the decision was, and that reads underneath as the
 * confidence, the way it does for a choice and a score.
 */
export function NoulMark({ value, yes, no }: { value: number; yes: string; no: string }) {
  return <p className="font-display text-[28px] font-semibold leading-snug text-ember">{value >= 0.5 ? yes : no}</p>;
}
...
/** What a probability of 1 reaches, in pixels: the band a full bar fills. */
const BAND = 64;
...
export function ScoreMark({
  ...
  return (
    <div>
      <p className="font-display text-[34px] font-semibold leading-none tabular-nums text-ember">{score.toFixed(2)}</p>
      {/* As tall as the distribution needs and no taller: the band is the
          tallest bar, so a flat score no longer reserves a full-height strip
          of empty. Its height comes from the peak rather than from the bars,
          which are 0 until they grow — otherwise the reveal would reflow the
          page under itself. */}
      <div className="mt-3 flex items-end gap-1" style={{ height: `${Math.max(0, Math.min(1, peak)) * BAND}px` }} aria-hidden>
        {levels.map((level) => (
          <span key={level.index} className="flex min-w-0 flex-1 flex-col justify-end">
            {/* Sized in pixels off the probability itself, on the same 0-1
                scale a choice's rows use: normalizing to the peak would make
                every winning level full height, and a flat distribution should
                look flat. */}
            <span
              className="bar-grow w-full bg-ember"
              style={fill(level.probability, {
                height: grown ? `${Math.max(0, Math.min(1, level.probability)) * BAND}px` : "0px",
                opacity: 0.85,
              })}
            />
          </span>
      ...
      <div className="relative mt-1.5 h-4">
        ...
        {/* n + 1 ticks for n levels: a level is the band *between* two of
            them, the way a histogram's columns sit between its boundaries. */}
        {levels.map((level) => (
          <span
            key={level.index}
            className="absolute top-0 h-1.5 w-px bg-line"
            style={{ left: `${(level.index / levels.length) * 100}%` }}
            aria-hidden
          />
        ))}
        {/* The closing one. At `left: 100%` it would fall a hair past the
            edge and never draw, so it is pinned to the edge from the inside. */}
        <span className="absolute top-0 right-0 h-1.5 w-px bg-line" aria-hidden />
 ...
 *
 * It draws the picture and nothing else. What the halo *means* is said by
 * whoever places this, under the whole answer rather than under the picture:
 * it is a caveat about the figures beside it as much as about the drawing.
 */
  ...
  return (
    <>
      <button
        ...
        // Centred in its own cell: the caption below it is as wide as the
        // column, so the figure does not shrink to the picture.
        className="group relative mx-auto block w-fit max-w-full cursor-zoom-in"
      >
        {plate("max-h-56")}
 ...
 *
 * White over black, and never a hue: the mark lands on a picture nobody chose
 * for it, so it has to read on a white background and a dark one alike — the
 * ember is spent on the halo, which is data, not on the cursor. The strokes
 * carry `non-scaling-stroke`, so a 2px line is 2px on screen whether the
 * picture is shown at 224px or at its own size; without it the same mark
 * vanished into a sub-pixel hairline in the preview.
 *
 * The arms leave a gap at the centre, so the answer's own pixel is the hole
 * rather than something covered by ink.
 */
export function PointGlyph({
  ...
  return (
    <>
      ...
      <circle cx={x} cy={y} r={gap} fill="none" stroke="#fff" strokeWidth={1.5} vectorEffect="non-scaling-stroke" />
    </>
  );
}
