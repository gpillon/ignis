...
- **Schedule** — one **permitted set** per token a **constrained decode**
  ...
  drawn from a set of digits cannot be one.
- **Terminator** — a token a **schedule** names, permitted by its steps like
  ...
  reader must get right: ended on the terminator (complete), spent its
  schedule without one (at the cap, complete), or stopped without one and
  short (the engine cut it off, an error) — told apart by the **last token**
  and never by the length. Not the request **scheduler's** anything — it schedules alphabets,
