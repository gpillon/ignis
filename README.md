...
| `IGNIS_RETAINED_SLOTS` | `--retained-slots <n>` | — | one per decode lane (`N_DECODE_LANES`); `0` with `--prompt-reuse off` | Retained slots reserved at load (ADR 0030): where every retained prompt checkpoint and shared prefix keeps its state image. When none is free, retained state gives one up — checkpoints before retained prefixes, `agent` before `interactive`, then least recently used; when nothing can, the publish or capture is skipped. Needs `--prompt-reuse on`. Replaces the removed `--retained-pool-bytes`. |
