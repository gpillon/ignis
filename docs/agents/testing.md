...
- Kernel work (`kernel/`, CUDA) is not exercised by `cargo test`:
  build with `kernel/build.ps1` and verify through the `ignis-bench`
  trace-replay / canary harness.
...
3. kernel changes: `kernel/build.ps1` clean + `ignis-bench` harness green