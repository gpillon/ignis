# Prefill chunk wall time is 90% per-layer compute; the forced synchronization costs 0.5%
...
**Branch this was measured on:** `issue-92-chunk-decomp`.
...
### Instrumentation and tooling built for this measurement
...
Five measurements, deliberately overlapping, so no single instrument had to be
...
### Measurement 1 — chunk-width sweep
...
Regressing wall on chunk count over all six widths gives a slope of
**17.7 ms per boundary**, which would be 141 ms of the default route's 790 ms.
Fitting only the widths whose per-token cost has plateaued (>= 1024) gives a
slope of **2.66 ms**, of which 0.47 ms is the synchronization (see Measurement
2). Both regressions are reported here as evidence; which one is valid is
addressed in Finding.

### Measurement 2 — leaf event decomposition
...
- `entry_gap`: device idle between the previous chunk's last operation and
  this chunk's first — the bubble the forced synchronization opens.
- `sync_stall`: host wall blocked inside `cudaStreamSynchronize` while the
  device computes — this is *not* device idle.
- `enqueue`: host time issuing the chunk's kernels; overlaps device work.
- `layer_gap`: device idle *between* layer bodies.

Instrumentation cost is inside run-to-run noise: 785.3 ms at width 1,024 with
the events on against 797.8 ms with them off.

### Measurement 3 — Nsight Systems
...
### Measurement 4 — the packing proxy (isolated-span sweep)

Each span here is prefilled on its own, from position zero, in exactly one
chunk of its own width — unlike Measurement 1, where a narrow chunk inside an
8,192-token span still attends to up to 8K of KV prefix, attention work a
standalone request of that width would never do.
...
idle from Measurement 3 (roughly fixed, since a traversal is ~1,160 kernels
...
### Measurement 5 — hardware counters (Nsight Compute)
...
of Measurement 4: flat at 17.4 GiB and rising to 19.88 GiB at the widest
traversal. All the growth is the prefill scratch arena, which
`ignis_model_load` sizes for the chunk width (P2-01, #83), about 2.5 GiB
across the sweep.

The KV pool is not the driver. For this budget — 258 page groups of 64
tokens, 16 GQA layers, 4 KV heads x 256 head dim, K and V, BF16:
...
**Observed (4.1, the per-chunk decomposition).** At the production 1,024-token
chunk width, a chunk is 98.6 ms of device timeline:
...
was opened on does not survive measurement. The route is compute-bound in the
sense that matters for that question.

**Inferred.** Why #86 changed nothing follows from this split: the
tensor-core routes landed inside the 90 % that was already compute, and what
they saved was not large relative to it. The residual 10 % is launch latency
spread over ~1,160 kernels per chunk, not the one synchronization per chunk.

**Observed/Inferred (Measurement 1, the regression trap).** The naive
regression across all six widths — 17.7 ms per boundary, which would be 141 ms
of the default route's 790 ms — is wrong. At 256 and 512 tokens the chunk is
too narrow for the GEMM shapes, so per-token device compute itself rises, and
the regression charges that lost compute to the boundary. Fitting only the
widths whose per-token cost has plateaued (>= 1024) gives the correct slope,
**2.66 ms**, of which 0.47 ms is the synchronization.

**Inferred.** This is also the answer to #110's chunk=512 observation from the
other side: a worse tail at the narrower width is consistent with a fixed
per-boundary cost, but equally consistent with narrow chunks computing less
efficiently, and the per-token compute column says it is the second.

**Observed (Measurement 2, leaf events).** `entry_gap` is the measurement #92
actually asked for: device idle between chunks, the bubble the forced
synchronization opens. **0.47 ms.** `sync_stall` is **not** that bubble — it
is host wall blocked inside `cudaStreamSynchronize` while the device computes.
Reading `sync_stall` as overhead is the mistake the ticket rested on: host
being blocked and device being idle are different quantities, and only the
second costs anything. `enqueue` overlaps device work; at width 8,192 it
absorbs the whole span with a 0.33 ms stall, which is a host throttled by a
full launch queue, not host-bound dispatch. `layer_gap` — device idle between
layer bodies — is 0.10 ms, essentially nothing: the host is never late with
the next layer.

**Observed (Measurement 3, Nsight Systems).** 90.2 % of the device timeline is
a kernel executing, with no reliance on the leaf's own events — this
independently confirms 4.1. The wider gaps are scattered across many kernel
types, ~11 per chunk, with no single structural host round-trip behind them.
The capture's own wall matching the uninstrumented sweep means tracing is not
inflating the idle it reports.

**Observed/Inferred (Measurement 4, the packing proxy).** "Busy" is not
"saturated": a GEMM occupying a quarter of the machine still counts as 100 %
busy on a timeline, so the 90.2 % finding must be read narrowly — **the wall
time is in kernels, not in synchronization or dispatch** — not as a claim that
the math is near peak. Packing N requests gives every projection and FFN GEMM
the shapes of one N*L-token traversal, and those GEMMs are ~75 % of kernel
time. Per-token cost bottoms out at **~1,024 tokens per traversal**. Four
256-token prompts in one traversal are **1.77x cheaper per token** than four
separate ones. Above 1,024 the curve turns back up, but that rise is
attention over a longer average prefix, so the wide end is a conservative
floor for packing rather than a ceiling. Splitting the 1.77x by mechanism:
...
**Observed/Inferred (Measurement 5, hardware counters).** At 256 tokens two of
the four dominant GEMMs launch **80 blocks onto a 170-SM card** — under half
the machine has any work — at ~20 % SM throughput. Filling the traversal
fixes both halves at once: the grids double, *and* the engine crosses into the
TMA route, taking SM throughput to 58–65 % and the tensor pipe to 66–75 %.
**The route switching with token count is a finding in its own right.** The
1.77x of Measurement 4 is two effects stacked, a wider grid and a kernel swap,
and neither is the synchronization this ticket set out to chase. Nothing is
saturated even at 1,024 tokens: `<5120,17408>` is still at 0.94 waves, so ~10
SMs sit idle through it, and the tensor pipe tops out at 75 %. But the wall
clock says pushing past 1,024 tokens does not convert that headroom into
speed, so the limiter above that width is inside the kernels, not in how many
tokens the traversal carries.

**Observed (VRAM audit).** All growth in leaf VRAM across the sweep is the
prefill scratch arena (about 2.5 GiB across the sweep, sized for chunk width
at load, P2-01/#83), not the KV pool, which is a flat ~1.2 GiB in every row.

## Implications

- **Criterion 1 of #92 is settled:** per-chunk wall time is **90 % per-layer
...
- **`.scratch/DEFERRED-DECISIONS.md` item 1 needs restating rather than
  ...
  prompts and nothing at 1,024 and above. The phase turns on the workload, not
  on this ticket's overhead numbers. The gates measure 8K prompts, where one
  request already fills a traversal; G4's subagent-burst trace is many short
  prompts, which is the regime that pays.
...
- **The preflight cannot tell a quiet card from a slow one, and this bit.**
  ...
  Every number in this document comes from the earlier session, where the
  same control reproduced its historical value. **Re-take the control
  diagnostic before trusting any new reading against these tables.**
...
- **Whether the 90 % itself can be made smaller is not settled.** The counters
  say no kernel is at peak (tensor pipe 66–75 % at the best width) but the
  wall clock says more tokens do not convert that headroom into speed, so the
  remaining limiter is inside the kernels. That is kernel work, not this
  ticket.
