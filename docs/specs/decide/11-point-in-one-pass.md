...
- **Production, measured** (`docs/findings/2026-09-22-the-codec-costs-the-head-its-read.md`):
  ...
  **The pre-registered criterion fails** — head 212, guard 222 of 240 —
  ...
  head is 232, BF16's number. In ignis every hq key attention read was
  then a codec decode, because the residual window was never wired. **With
  the window wired (#257, #258) the floors are met**: head 227, guard 233 —
  the window keeps 60% of a 1024 px image exact. An exact copy of the one
  KV head the head reads, over the image span, taken where the tap reads
  (after `qk_norm_rope`, before the append; 8 MB per sequence at 4096 px, a
  per-slot reserve in the VRAM plan), would buy the remaining 5 scenes at
  1024 px and 2 at 4096 px: a margin, not a precondition.
