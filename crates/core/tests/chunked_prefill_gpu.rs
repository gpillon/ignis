...
//! rest on them later. The chunked-vs-per-token self-oracle lives in
...
/// Long enough to span several chunks at every chunk width these tests use,
...
fn a_span_split_at_a_non_chunk_boundary_matches_one_span() {
    const PREFILL_CHUNK: u32 = 512;
    // Not a multiple of `PREFILL_CHUNK`, by construction.
    const SPLIT_AT: usize = 700;
...
#[test]
#[ignore = "GPU profile only: scripts/gpu-profile.ps1"]
fn the_prefill_chunk_width_is_a_performance_knob_not_a_correctness_parameter() {
    ...
    for chunk in [NARROW_CHUNK, WIDE_CHUNK] {
        ...
        let pool = SeqPool::create(
            &ModelConfig::qwen38_27b(),
            &SeqPoolBudget {
                kv_page_group_count: pages_for(MAX_CONTEXT),
                max_context_tokens: MAX_CONTEXT,
                slot_count: 1,
            },
        )
        .unwrap_or_else(|e| panic!("seq pool create(chunk={chunk}): {e}"));
        match prefill_whole_then_decode(&model, &pool, &span, vocab) {
            Ok(r) => results.push(r),
            Err(e) => {
    ...
    assert_eq!(
        ...
        argmax(wide_logits),
        "a {NARROW_CHUNK}-token and a {WIDE_CHUNK}-token prefill chunk must predict the same next \
         token for the same prompt"
    );
    assert_eq!(
        narrow_decoded, wide_decoded,
        "a {NARROW_CHUNK}-token and a {WIDE_CHUNK}-token prefill chunk must decode the same first \
         tokens for the same prompt"
    );
}

/// Two fresh model loads chunk-prefilling the same multi-chunk-wide prompt
/// through the default (chunked) route must produce identical greedy
/// tokens. `program_full_gpu.rs`'s own determinism test predates the
/// chunked route and uses a single-chunk-wide prompt; this is the
/// multi-chunk case the issue calls for.
#[test]
#[ignore = "GPU profile only: scripts/gpu-profile.ps1"]
fn chunked_prefill_of_a_long_prompt_is_deterministic_across_fresh_loads() {
    const PREFILL_CHUNK: u32 = 512;
