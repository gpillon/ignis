...
use crate::ttft::{self, generate_cell_prompts, PromptTemplate};
...
pub fn measure_throughput_cell(
    ...
    let set = match generate_cell_prompts(template, prompt_tokens as usize, concurrency) {
        Ok(set) => set,
...
/// each request's completion (`ep.complete` returning) *is* that
/// prefiller's release, so the next one is only sent once the previous is
/// fully done. [`std::thread::scope`] joins the decode lanes before
/// returning, so `ep` / `template` need not be `'static`.
pub fn measure_itl(ep: &dyn Endpoint, template: &dyn PromptTemplate, cfg: &ItlConfig) -> ItlCell {
    ...
    let decode_set = match generate_cell_prompts(
        template,
    ...
    let prefill_set = match generate_cell_prompts(
        template,
    ...
    let (lanes, prefillers) = std::thread::scope(|scope| {
        ...
        // thread. Each is allocated, prefilled and released — a completed
        // `ep.complete` call — before the loop sends the next.
...
pub fn measure(
    ...
    let c1 = measure_throughput_cell(
        ep,
        template,
    ...
    let c4 = measure_throughput_cell(
        ep,
        template,
        cfg.throughput.prompt_tokens,
        cfg.throughput.max_tokens,
    ...
    let itl = measure_itl(ep, template, &cfg.itl);
    Record {
